...
async function withLayerPanel(page: Page, act: (menu: Locator) => Promise<void>): Promise<void> {
...
async function layerRowText(page: Page, layer: number): Promise<string> {
  let text = ''
...
async function activateLayer(menu: Locator, layer: number): Promise<void> {
  const row = layerRow(menu, layer)
  await row.click()
  await expect(row).toHaveAttribute('aria-checked', 'true')
}

async function toggleLayer(menu: Locator, name: string): Promise<void> {
...
async function tap(page: Page, spot: Spot): Promise<void> {
...
// 途中の絵を掴まないよう落ち着くまで待つ
async function changedHash(page: Page, before: string): Promise<string> {
  let last = ''
  await expect
    .poll(async () => {
...
async function expectHashStays(page: Page, expected: string): Promise<void> {
...
const MAX_STRAY_PIXELS = 1000

type PixelStore = { __drawE2e?: Record<string, Uint8ClampedArray> }

async function rememberPixels(page: Page, key: string): Promise<void> {
...
async function strayPixels(page: Page, key: string): Promise<number> {
...
async function touchStroke(page: Page, secondFinger: boolean): Promise<void> {
