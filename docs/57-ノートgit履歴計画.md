...
- git 操作は [notesRepo.ts](../src/lib/git/notesRepo.ts) がプロセス内キューで
  ...
  (多重起動は範囲外)。
- author は固定 (`qr-note <qr-note@localhost>`)。シングルユーザーなので
  帰属を分ける相手がいない。複数ユーザー化するときにアプリのユーザー情報を
  入れる (§8)。改名 (QR search → QR Note) 前のコミットは `qr-search` 名義で
  残る。値はリポジトリのローカル設定に持たせ、**食い違っていたら書き直す** —
  作った当時の名前が焼き付くと、定数を変えても既存のリポジトリ (= 本番) が
  古い名義で署名し続けるため。合っているときに書かないのは、履歴を読むだけの
  操作を書き込み失敗の巻き添えにしないため。
