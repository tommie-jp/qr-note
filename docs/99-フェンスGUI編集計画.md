...
計画日: 2026-09-23 / 状態: **実装済み (Phase 1・2、circuit は docs/100、破棄の釦は §10)**。iPhone 実機は未 (§9)
...
| 6 | 殻の中の undo は殻自前 (`undo: 'own'`)。**閉じる = 当てる。** ~~捨てる釦は持たない~~ → 2026-09-24 に「破棄」を足した (§10) | VS Code のカスタムエディタも操作のたびに書く。しくじりは殻の中の元に戻すで足り、閉じた後は CodeMirror の元に戻す 1 段で全部戻る。ただし帯には「閉じると本文に反映します」としか出ず、戻せることが見えない。捨てたい人が閉じるのをためらうので、破棄の釦を置いた |
| 7 | 器は DrawModal 型の全画面 (`fixed inset-0 z-50`、上の帯に「図を編集」と 破棄・閉じる)。**safe-area は器で取る**。iframe は `flex-1`、器に `touch-none` は付けない | 52 の docs/50: iframe の中の `env()` はブラウザで値が違う。ピンチとパンは iframe の中で捌くので、器が指を取ってはいけない |
...
| 破棄 | 帯に題・案内・破棄・閉じるが 1 行で収まる。変えていれば確認が出て、了承すると本文は変わらない | `FenceGuiModal` |
...
- **iPhone 実機** (§6 の表)。とくに長押しのメニューと、属性の欄を押したときだけ
  キーボードが出るか
- ~~**circuit (YAML)**~~ — [100](100-回路図を図を編集に足す計画.md) で実装した
  (0.8.0 の core に editor が入っており、版を上げずに足せた)
...
ユーザーから「図を編集したあと、編集内容を捨てて QR ノートに戻れるようにしたい」と
頼まれた。決め 6 では捨てる釦を持たなかった。閉じてから元に戻す 1 回で同じ結果に
なるが、帯の案内からはそれが読めない。

| 何 | どうしたか |
| --- | --- |
| 並び | 帯の「閉じる」の左に「破棄」。「閉じる」は今までどおり本文に当てる。釦の意味を裏返すと、v0.23.14 から使っている手の動きが逆になる |
| 名前 | 「破棄」の 2 文字。iPhone の帯は題・案内・釦で幅が詰まっている。図を編集の画面の中なので「戻る」は言わなくても伝わる |
| 確認 | 殻の写しが開いたときの本文と違うときだけ `window.confirm`。殻の履歴は iframe と一緒に消えるので、捨てたら戻せない。DrawModal の「描いたものは保存されません」と同じ作法。何も変えていなければ黙って閉じる |
| Esc | 閉じる (当てる) のまま。押し間違えても元に戻すで戻せる側に倒す |
| 当て方 | 破棄は `onClose` に開いたときの本文をそのまま渡す。`commitSpec` が `unchanged` を返すので何も dispatch しない。フック (`useFenceGui`) と橋 (`openFenceGui`) は触っていない |
| 釦を無効にするか | しない。変わったかを器の state に引く配線が要る。確認を変更時だけにすれば同じ安全さになる |

試験は `e2e/fence-gui.spec.ts` に 3 本足した (板の図で)。

- 値を直して破棄し、確認で了承すると本文は変わらない (確認の文言も見る)
- 破棄の確認を取り消すと殻は開いたままで、閉じれば反映される
- 何もせずに破棄すると確認なしで閉じる

E2E は図を編集の 11 本がすべて通った。iPhone 実機は §6 の表の「破棄」の行が残り。
