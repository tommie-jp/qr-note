...
## 追記 (2026-09-21): 読めたところまで描く

circuit-fence 0.8.0 で **読めない行があっても組めたところまで描く**ようになり、
QR ノート側もそれに合わせた (実体配線図 docs/97 と同じ扱い)。以下の本文は
**0.3.0 当時の姿**として残してあるので、いま読むときは次の 3 点を差し引くこと。

- **`errors` があっても図は出る。** 失敗 (`{ error, texLog }`) に落ちるのは
  `compileCircuit` の `tex` が `null` のとき — 部品が 1 つも立たず、描くものが
  無い図だけ。§2 の「コンパイルエラーは TeX まで行かずに返せる」はこの場合を指す
- **読めなかった行は成功の側に `errors` として載る** (`CircuitResult`)。
  `notices` と違って `style: debug: off` では伏せない — 直さない限り図が
  変わらないため。図の下の帯は赤 (読めなかった行) が先、琥珀 (お知らせ) が後
- **編集画面のライブプレビューとオフラインには帯が出ない。** どちらも図だけを
  見せる (実体配線図と同じ)。帯を足すなら 3 つのフェンスで揃えて足すこと
...
段取り 1〜5 を通した。`circuit-fence` は **0.2.1** を vendor に置いている
(0.2.0 から `style: debug: on/off` が増えた。図そのものは 1 ドットも変わらない)。

### 決めたこと (計画から変えたところ)

- **お知らせ (notices) は閲覧にも出す。** 検討の途中では「書き手にしか
  直せないのだから編集画面だけ」に倒しかけたが、0.2.1 で書き手が図ごとに
  伏せられるようになったので、判断の軸が「どの画面か」ではなく
  「承知のうえでそう書いたか」に移った。画面で出し分けると、書き手が
  「off にしたのに閲覧では出る」を追えなくなる。
  **伏せるのは出す側の仕事** — core は off でも notices を返し続けるので、
  こちらで落とさないと「出さない」が「無かったことにする」に化ける。
- **描画結果の鍵に言語を混ぜた** (`circuitKey`)。同じ本文が 2 つの言語で
  書かれても別の図なので、本文だけを鍵にすると片方の図がもう片方の場所に出る。
  DB のキャッシュキー (hash) とは別物で、あちらは版を混ぜる。
- **定数は `CIRCUITIKZ_LANG` / `CIRCUIT_LANG`** (§3 の推奨案)。改名だけを
  先に 1 コミットにして、意味の入れ替えと混ざらないようにした。
- **`CIRCUIT_LANGS` と `circuitKey` は fenceLanguages に置いた。** 閲覧と
  エディタの両方が鍵を作るので、remark を抱えた circuitFences に置くと
  client のバンドルへ引きずり込む (葉モジュールの約束)。
- **YAML のハイライトは入れていない。** このエディタは*どのフェンスも*
  言語別の色付けをしておらず (codeLanguages 未設定)、ここだけ足すと
  mermaid や quiz と揃わない。要るときに全部まとめて決める。
- 書式メニューに「回路図 (YAML)」を足した (§4 の表には無いが、
  足さないと新しいフェンスに気づく入口が無い)。

### 持ち出しの形が変わった

`OfflineCircuit` に `lang` が増えた。**古い保存には無い**ので、欠けていたら
`circuitikz` とみなす。知らない綴りも同じ扱い — 図は既に描けているので、
言語を読み損ねただけで持ち出しを捨てる理由がない。

### 確かめたこと (2026-08-31 実測)

- [denken/](https://github.com/tommie-jp/49-circuit-fence) の図 **57 枚が
  エラーゼロ**で compileCircuit を通る。お知らせも 0 件
- そのうち 1 枚を **TeX → SVG まで実際に描き**、仕上げ (注釈・塗り替え・
  刻印) と `assertSafeCircuitSvg` を通した (4,472 バイト、刻印 0.2.1)
- 注釈の日本語を差し込んだ SVG が許可リストを通ることを単体テストに残した
  (§2 の「実装時にもテストとして残す」)
- lockfile で node-tikzjax が 1 つに dedupe されている (二重だと jsdom ごと
  87MB 増える)
- 本番ビルド (`npm run build`) が通る

### 残り

- 本番ノートへ図を戻す経路 (denken の Phase 3)。手で編集するか、
  エクスポート zip を書き換えて import API に流すか
