...
test('notes/*.md を番号ごと戻す', async () => {
  const report = await importZip(zipSync({ 'notes/1042.md': noteFile('1042') }))
...
test('作成・更新日時を書き戻す', async () => {
  await importZip(zipSync({ 'notes/1042.md': noteFile('1042') }))
...
test('公開状態もファイルに合わせる', async () => {
  ...
  await importZip(zipSync({ 'notes/7.md': file }))
...
test('想定外のパスは黙って読み飛ばさず理由を載せる', async () => {
  const report = await importZip(
    zipSync({ 'secret/passwd': strToU8('x'), 'README.md': strToU8('x') }),
  )
...
test('保存できなかった添付を理由付きで載せる', async () => {
  ...
  const report = await importZip(
    zipSync({ [`images/${UUID}.jpg`]: strToU8('<html>') }),
  )
...
test('本文が指す添付が ZIP にも DB にも無ければ理由を載せる', async () => {
  const report = await importZip(
    zipSync({ 'notes/1042.md': noteFile('1042', `![](../images/${UUID}.jpg)`) }),
  )
...
test('1 件のノートの失敗で ZIP 全体を落とさない', async () => {
  ...
  const report = await importZip(
    zipSync({ 'notes/1.md': noteFile('1'), 'notes/2.md': noteFile('2') }),
  )
...
test('同じ名前の添付が既にあれば数に入れない', async () => {
  ...
  const report = await importZip(
    zipSync({ [`images/${UUID}.jpg`]: new Uint8Array([1, 2, 3]) }),
  )
...
test('ZIP として読めないファイルは例外 (ファイルごと対象外)', async () => {
  await expect(importZip(strToU8('ただのテキスト'))).rejects.toThrow(
    /ZIP ファイルではありません/,
  )
})
