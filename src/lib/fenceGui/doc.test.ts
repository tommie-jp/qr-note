...
import { applyChanges, docOver, replaceLines } from './doc'

// 上流 playground の doc.test.ts の写し (docs/99)。殻は Markdown の全文を
// 「文書」として読み、行と桁で書き換える。**写しが崩れると QR ノートだけで
// 壊れて上流では気づけない**ので、試験も同じものを持つ
...
describe('replaceLines', () => {
  ...
  test('本文が 0 行のフェンスには差し込む', () => {
    ...
    const out = replaceLines(empty, 3, 0, ['board: 25x15'])
